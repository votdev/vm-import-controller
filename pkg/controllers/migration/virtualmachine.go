...
const (
	...
	defaultShutdownGuestTimeout    = 5 * time.Minute
)
...
func triggerShutdownGuest(vm *migration.VirtualMachineImport, vmo VirtualMachineOperations) error {
	logrus.WithFields(logrus.Fields{
		...
		"spec.shutdownGuestTimeout": vm.Spec.ShutdownGuestTimeout,
	}).Info("Shutting down guest OS of the source VM")
	err := vmo.ShutdownGuest(vm)
	if err != nil {
		return fmt.Errorf("failed to shut down guest OS off the source VM: %v", err)
	}
...
func (h *virtualMachineHandler) triggerExport(vm *migration.VirtualMachineImport) error {
	...
	if !util.ConditionExists(vm.Status.ImportConditions, migration.VirtualMachinePoweredOff, v1.ConditionTrue) && (util.ConditionExists(vm.Status.ImportConditions, migration.VirtualMachinePoweringOff, v1.ConditionTrue) ||
		...
		if ok {
			...
			if vm.Spec.ShutdownGuest {
				// If the VM was not powered off by shutting down the guest
				// OS within the configured time period, then force a hard
				// power off.
				...
				if condition != nil {
					...
					shutdownGuestTimeout := vm.Spec.ShutdownGuestTimeout
					if shutdownGuestTimeout == 0 {
						shutdownGuestTimeout = defaultShutdownGuestTimeout
					}

					if time.Since(lastUpdateTime) > shutdownGuestTimeout {
						logrus.WithFields(logrus.Fields{
							"name":                      vm.Name,
							"namespace":                 vm.Namespace,
							"spec.virtualMachineName":   vm.Spec.VirtualMachineName,
							...
							"spec.sourceCluster.name":   vm.Spec.SourceCluster.Name,
							"spec.shutdownGuestTimeout": vm.Spec.ShutdownGuestTimeout,
							"shutdownGuestTimeout":      shutdownGuestTimeout.String(),
						}).Info("Forcing power off of the source VM because the guest OS did not shut down within the configured time period")
						return triggerPowerOff(vm, vmo)
					}
				}
			}
